...
export function CandidateProfile({
  ...
  return (
    <section className="bg-zinc-950 border border-zinc-800 rounded-lg p-4 sm:p-6 space-y-5 sm:space-y-6">
      <div>
        <h2 className="text-base sm:text-lg font-bold text-white flex items-center gap-2">
          <UserCheck className="w-5 h-5 text-emerald-400 shrink-0" /> Candidate Profile & AI Job Matching Application Form
        </h2>
      ...
      <div className="p-4 sm:p-5 bg-black border-2 border-dashed border-zinc-800 hover:border-zinc-600 rounded-lg text-center transition space-y-3 relative">
        <input
          type="file"
        ...
        {cvSuccessMsg && (
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-950/80 border border-emerald-800 text-emerald-300 text-[11px] rounded font-mono max-w-full text-left">
            <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> <span className="truncate">{cvSuccessMsg}</span>
          </div>
        )}
      </div>
      ...
      {profileMsg && (
        <div className="p-3 bg-zinc-900 border border-zinc-700 rounded text-xs text-white flex items-center gap-2 font-mono">
          <AlertCircle className="w-4 h-4 text-zinc-400 shrink-0" /> {profileMsg}
        </div>
      )}

      <form onSubmit={handleSaveProfile} className="space-y-4">
        {/* Natural Language Job Preferences Section */}
        <div className="p-3.5 sm:p-4 bg-zinc-900/60 border border-zinc-800 rounded-md space-y-2">
          <label className="block text-xs font-semibold text-white flex items-center gap-1.5 font-mono uppercase tracking-wide">
            <Sparkles className="w-3.5 h-3.5 text-amber-400 shrink-0" /> AI Job Preferences & Target Position Criteria (Auto-Generated)
          </label>
        ...
        <div>
          <label className="block text-xs font-medium text-zinc-400 mb-1 flex flex-col sm:flex-row sm:items-center justify-between gap-1">
